...
#define __RANGE_FAR 800
#define __RANGE_CLOSE 100
#include "script_component.hpp"

if ((_this select 4) isKindOf "BulletCore" || {isNull (_this select 6)}) exitWith {};
...
_this spawn {
	...
	switch (true) do {
		...
		case (_ammo isKindOf "GrenadeBase" || {_ammo isKindOf "TimeBombCore"} || {_ammo isKindOf "FuelExplosion"} || {_ammo isKindOf "MissileCore"} || {_ammo isKindOf "RocketCore"} || {_ammo isKindOf "ShotDeployCore"} || {_ammo isKindOf "ShellCore"} || {_ammo isKindOf "BombCore"} || {_ammo isKindOf "SubmunitionCore"}): {
			_range = __RANGE_FAR;
		};
		case (_ammo isKindOf "SmokeShellCore" || {_ammo isKindOf "FlareCore"}): {
			_range = __RANGE_FAR;
			_early = true;
		};
	};

	if (_range < __RANGE_CLOSE) exitWith {TRACE_1("short range, exiting",_range)};

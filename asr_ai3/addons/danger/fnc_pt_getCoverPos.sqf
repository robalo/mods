...
params ["_unit", "_dangerUnit", "_coverObj"];
...
if (_originalCoverPos isEqualTo []) exitWith {
    TRACE_1("cover rejected, unable to find center", _originalCoverPos);
    [];
};
...
while {_resolution > 0.25} do {
    ...
    if (GVAR(debug_findcover)) then {
        if (isNil "unitPts") then {
            unitPts = [];
            addMissionEventHandler ["Draw3D", { {drawLine3D [_x select 0, _x select 1, [1, 0, 0, 1]]} forEach unitPts; }];
        };
...
if (_foundResult isEqualTo 0) exitWith {
    TRACE_1("Cover rejected during sweep", _coverObj)
    [];
};
...
if (GVAR(debug_findcover)) then {
    if (isNil QGVAR(coverPts)) then {
        GVAR(coverPts) = [];
        addMissionEventHandler ["Draw3D", { {drawLine3D [_x, _x vectorAdd [0,0,10], [0,1,0,1]]} forEach GVAR(coverPts); }];
    };
    GVAR(coverPts) pushBack (ASLToATL _coverPos);
};
_coverPos;

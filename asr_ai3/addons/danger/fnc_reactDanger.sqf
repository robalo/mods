...
if (_unit call FNCMAIN(isValidUnitC) && {unitReady _unit} && {_grp call FNCMAIN(hasNoPlayer)} && {_time > (_unit getVariable [QGVAR(reacting),-1000]) + 60}) then {
	...
	if (!isNull _dangerCausedBy && {random 1 < GVAR(usebuildings)}) then {
		...
		if (count _bpos > 0) then {
			...
			[_dude,(_time + 240),_bpos] spawn { // 4 minutes
				...
				while {count _bpos > 0 && {time < _dangerUntil} && {_dude call FNCMAIN(isValidUnitC)}} do {
                    if ((group _dude) call {_this call FNCMAIN(hasPlayer) && {_this call FNCMAIN(grpHasWP)}}) exitWith {}; //crossing paths issue
					...
					if (_dude call FNCMAIN(isUnderRoof)) then {_dude setUnitPosWeak "Up"} else {_dude setUnitPosWeak "Auto"};
					doStop _dude;
					sleep (5 + random 20);
				};

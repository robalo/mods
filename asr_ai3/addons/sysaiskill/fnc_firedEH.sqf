...
/*
			...
			if (_burst > 3) then {
				...
				if (!isPlayer _shooter) then {
					if (_veh == _shooter) then {
						if (isNil {_shooter getVariable "asr_ai3_sysdanger_shooting"}) then {
...
*/
			...
			if (isPlayer gunner _veh || isPlayer driver _veh) then {
				...
				if ([_veh,"(forest + houses)",4] call FNCMAIN(isNearStuff)) then {
...

			{
				if (group _x != group _shooter) then {
					...
					if ([_x,"(forest + houses)",4] call FNCMAIN(isNearStuff)) then {
						_k = _k * 0.5;
					};
					_x reveal [_veh,_k];
